...
void
list_split_lines(struct list* L, const char* s)
{
  list_split(L, s, '\n');
}

void
list_split(struct list* L, const char* s, char delimiter)
{
  const char* p = s;
  const char* q;
  while (*p)
...
bool
list_get(struct list* target, int index, void** result)
{
  if (target->size == 0)
    return false;
...
void list_map(struct list* target, void (*callback)(void*))
{
...
void list_free_callback(struct list* target, void (*destructor)(void*))
{
  list_destruct(target, destructor);
  free(target);
}
